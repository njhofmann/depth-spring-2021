import os
import torch
from conv_ops.ffi import create_extension
import setuptools as s
import torch.utils.cpp_extension as cpp  # as create_extension

this_file = os.path.dirname(__file__)

sources = ['src/depthavgpooling.c']
headers = ['src/depthavgpooling.h']
defines = []
with_cuda = False

if torch.cuda.is_available():
    print('Including CUDA code.')
    sources += ['src/depthavgpooling_cuda.c']
    headers += ['src/depthavgpooling_cuda.h']
    defines += [('WITH_CUDA', None)]
    with_cuda = True

this_file = os.path.dirname(os.path.realpath(__file__))
print(this_file)
extra_objects = ['src/depthavgpooling_cuda_kernel.cu.o']
extra_objects = [os.path.join(this_file, fname) for fname in extra_objects]

# ffi = create_extension(
#     '_ext.depthavgpooling',
#     headers=headers,
#     sources=sources,
#     define_macros=defines,
#     relative_to=__file__,
#     with_cuda=with_cuda,
#     extra_objects=extra_objects
# )

if __name__ == '__main__':
    if not torch.cuda.is_available():
        raise ValueError('Please install CUDA for GPU support.')
    # ffi.build()
    s.setup(name='depthavgpooling',
            ext_modules=[cpp.CUDAExtension(name='_ext.depthavgpooling',
                                           sources=sources,
                                           extra_compile_args={'nvcc': [],
                                                               'cxx': ['-g']})],
            cmdclass={'build_ext': cpp.BuildExtension})
